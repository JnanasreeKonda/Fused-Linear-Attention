...
def kernel_forward(X_np, Wq_np, Wk_np, Wv_np, B, H, N, D, d_head, kernel, kernel_dtype):
    ...
    torch_dtype = torch.float16 if kernel_dtype == "float16" else torch.float32
...
def main():
    ...
    parser.add_argument("--kernel-dtype", default="float32", choices=["float32", "float16"])
    ...
    tolerance = FP16_TOLERANCE if args.kernel_dtype == "float16" else TOLERANCE
