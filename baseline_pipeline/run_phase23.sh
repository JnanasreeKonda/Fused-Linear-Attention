...
"$PYTHON_BIN" ../tests/test_correctness.py \
  --simulate \
  --quick \
  --use-root-oracle \
  --out results/correctness_results_phase23_quick.csv
...
"$PYTHON_BIN" profiling/fused_bench.py \
  ...
  --timed 20 \
  --warmup 5 \
  --out results/fused_profiling_phase23_sim.csv \
  --sweep-out results/occupancy_sweep_phase23_sim.csv
